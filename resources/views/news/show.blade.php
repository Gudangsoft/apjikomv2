@extends('layouts.main')

@section('title', $news->title)

@push('meta')
    <!-- Open Graph / Facebook -->
    <meta property="og:type" content="article">
    <meta property="og:url" content="{{ url()->current() }}">
    <meta property="og:title" content="{{ $news->title }}">
    <meta property="og:description" content="{{ $news->excerpt ?? Str::limit(strip_tags($news->content), 160) }}">
    @if($news->image)
    <meta property="og:image" content="{{ asset('storage/' . $news->image) }}">
    @endif

    <!-- Twitter -->
    <meta property="twitter:card" content="summary_large_image">
    <meta property="twitter:url" content="{{ url()->current() }}">
    <meta property="twitter:title" content="{{ $news->title }}">
    <meta property="twitter:description" content="{{ $news->excerpt ?? Str::limit(strip_tags($news->content), 160) }}">
    @if($news->image)
    <meta property="twitter:image" content="{{ asset('storage/' . $news->image) }}">
    @endif
@endpush

@section('content')
<!-- Article Header -->
<section class="bg-purple-600 text-white py-8">
    <div class="container mx-auto px-4">
        <div class="max-w-4xl mx-auto">
            <div class="mb-3">
                <span class="bg-white bg-opacity-20 px-3 py-1 rounded text-xs font-medium uppercase tracking-wide">{{ $news->category->name }}</span>
            </div>
            <h1 class="text-3xl font-bold mb-3">{{ $news->title }}</h1>
            <div class="flex items-center text-sm text-purple-100">
                <span class="mr-3">{{ $news->user->name }}</span>
                <span class="mr-3">•</span>
                <span class="mr-3">{{ $news->published_at->format('d F Y') }}</span>
                <span class="mr-3">•</span>
                <span>{{ $news->views }} views</span>
            </div>
        </div>
    </div>
</section>

<!-- Article Content -->
<section class="py-12 bg-gray-50">
    <div class="container mx-auto px-4">
        <div class="max-w-7xl mx-auto">
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div class="lg:col-span-2">
            <article class="bg-white rounded shadow-sm p-8 border">
                @if($news->image)
                <div class="mb-6 -mt-8 -mx-8">
                    <img src="{{ asset('storage/' . $news->image) }}" alt="{{ $news->title }}" class="w-full h-96 object-cover rounded-t">
                </div>
                @endif
                
                @if($news->excerpt)
                <div class="text-lg text-gray-600 italic mb-6 pb-6 border-b border-gray-200">
                    {{ $news->excerpt }}
                </div>
                @endif
                
                <div class="prose prose-lg max-w-none text-gray-700 leading-relaxed">
                    {!! $news->content !!}
                </div>
                
                <div class="mt-8 pt-6 border-t border-gray-200">
                    <div class="flex items-center justify-between flex-wrap gap-4">
                        <div class="flex items-center flex-wrap gap-3">
                            <span class="text-sm text-gray-500 font-medium">Bagikan:</span>
                            <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(url()->current()) }}" 
                               target="_blank" 
                               class="inline-flex items-center space-x-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm transition-colors">
                                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                                    <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                                </svg>
                                <span>Facebook</span>
                            </a>
                            <a href="https://twitter.com/intent/tweet?url={{ urlencode(url()->current()) }}&text={{ urlencode($news->title) }}" 
                               target="_blank" 
                               class="inline-flex items-center space-x-2 bg-sky-500 hover:bg-sky-600 text-white px-4 py-2 rounded-lg text-sm transition-colors">
                                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                                    <path d="M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723c-.951.555-2.005.959-3.127 1.184a4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.827 4.996 4.996 0 01-2.212.085 4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0024 4.59z"/>
                                </svg>
                                <span>Twitter</span>
                            </a>
                            <a href="https://www.linkedin.com/sharing/share-offsite/?url={{ urlencode(url()->current()) }}" 
                               target="_blank" 
                               class="inline-flex items-center space-x-2 bg-blue-700 hover:bg-blue-800 text-white px-4 py-2 rounded-lg text-sm transition-colors">
                                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                                    <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
                                </svg>
                                <span>LinkedIn</span>
                            </a>
                            <a href="https://wa.me/?text={{ urlencode($news->title . ' - ' . url()->current()) }}" 
                               target="_blank" 
                               class="inline-flex items-center space-x-2 bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded-lg text-sm transition-colors">
                                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                                    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z"/>
                                </svg>
                                <span>WhatsApp</span>
                            </a>
                        </div>
                    </div>
                </div>
            </article>
            </div>
            
            <!-- Sidebar -->
            <aside class="lg:col-span-1">
                <!-- Berita Terbaru -->
                <div class="bg-white rounded shadow-sm border p-6 mb-6 sticky top-24">
                    <h3 class="text-lg font-bold mb-4 text-gray-900 border-b pb-2">Berita Terbaru</h3>
                    <div class="space-y-4">
                        @foreach($latestNews as $latest)
                        <article class="group">
                            @if($latest->image)
                            <div class="mb-2 overflow-hidden rounded">
                                <a href="{{ route('news.show', $latest->slug) }}">
                                    <img src="{{ asset('storage/' . $latest->image) }}" 
                                         alt="{{ $latest->title }}" 
                                         class="w-full h-24 object-cover group-hover:scale-105 transition-transform duration-300">
                                </a>
                            </div>
                            @endif
                            <h4 class="font-semibold text-sm mb-1">
                                <a href="{{ route('news.show', $latest->slug) }}" 
                                   class="hover:text-purple-600 transition-colors">
                                    {{ Str::limit($latest->title, 60) }}
                                </a>
                            </h4>
                            <div class="flex items-center text-xs text-gray-500">
                                <span>{{ $latest->published_at->format('d M Y') }}</span>
                                <span class="mx-2">•</span>
                                <span>{{ $latest->views }} views</span>
                            </div>
                        </article>
                        @endforeach
                    </div>
                </div>

                <!-- Berita Populer -->
                <div class="bg-white rounded shadow-sm border p-6 mb-6">
                    <h3 class="text-lg font-bold mb-4 text-gray-900 border-b pb-2">Berita Populer</h3>
                    <div class="space-y-4">
                        @foreach($popularNews as $index => $popular)
                        <article class="flex items-start space-x-3 group">
                            <div class="flex-shrink-0 w-8 h-8 bg-purple-600 text-white rounded-full flex items-center justify-center font-bold text-sm">
                                {{ $index + 1 }}
                            </div>
                            <div class="flex-1">
                                <h4 class="font-semibold text-sm mb-1">
                                    <a href="{{ route('news.show', $popular->slug) }}" 
                                       class="hover:text-purple-600 transition-colors">
                                        {{ Str::limit($popular->title, 50) }}
                                    </a>
                                </h4>
                                <div class="flex items-center text-xs text-gray-500">
                                    <span>{{ $popular->views }} views</span>
                                </div>
                            </div>
                        </article>
                        @endforeach
                    </div>
                </div>

                <!-- Kategori -->
                <div class="bg-gradient-to-br from-purple-600 to-purple-700 rounded shadow-sm p-6 text-white">
                    <h3 class="text-lg font-bold mb-4 border-b border-white/20 pb-2">Kategori</h3>
                    <div class="space-y-2">
                        @foreach($categories as $category)
                        <a href="{{ route('news.index', ['category' => $category->slug]) }}" 
                           class="flex items-center justify-between py-2 px-3 rounded hover:bg-white/10 transition-colors">
                            <span class="text-sm">{{ $category->name }}</span>
                            <span class="text-xs bg-white/20 px-2 py-1 rounded">{{ $category->news_count }}</span>
                        </a>
                        @endforeach
                    </div>
                </div>
            </aside>
            </div>
            
            <!-- Related News -->
            <div class="max-w-7xl mx-auto mt-10">
            @if($relatedNews->count() > 0)
            <div class="mt-10">
                <h2 class="text-2xl font-bold mb-6 text-gray-900">Berita Terkait</h2>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    @foreach($relatedNews as $related)
                    <article class="bg-white rounded shadow-sm border hover:border-purple-600 transition-colors">
                        @if($related->image)
                            <div class="h-32 overflow-hidden">
                                <img src="{{ asset('storage/' . $related->image) }}" alt="{{ $related->title }}" class="w-full h-full object-cover">
                            </div>
                        @else
                            <div class="h-32 bg-gradient-to-br from-purple-500 to-purple-700"></div>
                        @endif
                        <div class="p-4">
                            <h3 class="font-semibold mb-2 text-sm">
                                <a href="{{ route('news.show', $related->slug) }}" class="hover:text-purple-600">
                                    {{ Str::limit($related->title, 60) }}
                                </a>
                            </h3>
                            <p class="text-xs text-gray-500">{{ $related->published_at->format('d M Y') }}</p>
                        </div>
                    </article>
                    @endforeach
                </div>
            </div>
            @endif
            </div>
    </div>
</section>
@endsection
