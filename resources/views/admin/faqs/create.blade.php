...
<div class="bg-white rounded-lg shadow p-6">
    <h3 class="text-2xl font-bold text-gray-900 mb-6">Tambah FAQ Baru</h3>
    ...
    <form action="{{ route('admin.faqs.store') }}" method="POST">
        ...
        <div class="mb-6">
            ...
            <input type="text" id="question" name="question" value="{{ old('question') }}" maxlength="500" required
        ...
        <div class="mb-6">
            ...
            <textarea id="answer" name="answer" rows="5" required
                      class="w-full px-4 py-2 border rounded focus:ring-2 focus:ring-purple-500 @error('answer') border-red-500 @enderror">{{ old('answer') }}</textarea>
            @error('answer')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div>
                ...
                <select id="category" name="category" required
                    ...
                    <option value="membership" {{ old('category') == 'membership' ? 'selected' : '' }}>Keanggotaan</option>
