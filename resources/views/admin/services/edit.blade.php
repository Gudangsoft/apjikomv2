...
<div class="container mx-auto px-4 py-6">
    ...
    <div class="mb-6">
        ...
        <p class="text-gray-600 mt-1">Perbarui informasi layanan {{ $service->title }}</p>
    </div>
        ...
        <form action="{{ route('admin.services.update', $service) }}" method="POST">
            ...
            <div class="mb-6">
                ...
                <select 
                    ...
                    required
                >
                    <option value="blue" {{ old('color', $service->color) == 'blue' ? 'selected' : '' }}>Biru</option>
                    <option value="purple" {{ old('color', $service->color) == 'purple' ? 'selected' : '' }}>Ungu</option>
                    <option value="green" {{ old('color', $service->color) == 'green' ? 'selected' : '' }}>Hijau</option>
                    <option value="orange" {{ old('color', $service->color) == 'orange' ? 'selected' : '' }}>Oranye</option>
                    <option value="red" {{ old('color', $service->color) == 'red' ? 'selected' : '' }}>Merah</option>
                    <option value="teal" {{ old('color', $service->color) == 'teal' ? 'selected' : '' }}>Teal</option>
                    <option value="indigo" {{ old('color', $service->color) == 'indigo' ? 'selected' : '' }}>Indigo</option>
                </select>
                @error('color')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="mb-6">
                ...
                <div id="features-container" class="space-y-2">
                    @php
                        ...
                        $featureCount = max(4, count($existingFeatures));
                    @endphp
                    @for($i = 0; $i < $featureCount; $i++)
                        <input 
                            type="text" 
                            name="features[]" 
                            value="{{ $existingFeatures[$i] ?? '' }}"
