    ...
    <form method="POST" action="{{ route('admin.events.update', $event) }}" enctype="multipart/form-data">
        ...
        <div class="mb-6">
            ...
            <div class="mb-3">
                ...
                <img src="{{ asset('storage/' . $event->image) }}" alt="{{ $event->title }}" class="max-w-xs rounded shadow">
            </div>
            @endif
        ...
        <div class="mb-6">
            ...
            <textarea id="description" name="description" rows="6" required
                      class="w-full px-4 py-2 border rounded focus:ring-2 focus:ring-[#00629B] @error('description') border-red-500 @enderror">{{ old('description', $event->description) }}</textarea>
            @error('description')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-6">
            ...
            <div class="grid grid-cols-3 gap-4">
                <label class="flex items-center p-4 border rounded cursor-pointer hover:bg-gray-50 {{ old('event_type', $event->event_type ?? 'offline') == 'offline' ? 'border-purple-500 bg-purple-50' : '' }}">
                    <input type="radio" name="event_type" value="offline" {{ old('event_type', $event->event_type ?? 'offline') == 'offline' ? 'checked' : '' }} 
                           class="text-purple-600" onchange="toggleEventFields()">
                    <span class="ml-2 font-medium">Offline</span>
                </label>
                <label class="flex items-center p-4 border rounded cursor-pointer hover:bg-gray-50 {{ old('event_type', $event->event_type) == 'online' ? 'border-purple-500 bg-purple-50' : '' }}">
                    <input type="radio" name="event_type" value="online" {{ old('event_type', $event->event_type) == 'online' ? 'checked' : '' }} 
                           class="text-purple-600" onchange="toggleEventFields()">
                    <span class="ml-2 font-medium">Online</span>
                </label>
                <label class="flex items-center p-4 border rounded cursor-pointer hover:bg-gray-50 {{ old('event_type', $event->event_type) == 'hybrid' ? 'border-purple-500 bg-purple-50' : '' }}">
                    <input type="radio" name="event_type" value="hybrid" {{ old('event_type', $event->event_type) == 'hybrid' ? 'checked' : '' }} 
                           class="text-purple-600" onchange="toggleEventFields()">
        ...
        <div class="mb-6" id="platformField">
            ...
            <input type="text" name="online_platform" value="{{ old('online_platform', $event->online_platform) }}"
        ...
        <div class="bg-gray-50 rounded-lg p-6 mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4">
                <div>
                    <label class="flex items-center space-x-2 cursor-pointer">
                        <input type="checkbox" name="has_registration" value="1" {{ old('has_registration', $event->has_registration) ? 'checked' : '' }}
                ...
                <div>
                    <label class="flex items-center space-x-2 cursor-pointer">
                        <input type="checkbox" name="has_certificate" value="1" {{ old('has_certificate', $event->has_certificate) ? 'checked' : '' }}
            ...
            <div id="certificateTemplateField" class="bg-purple-50 border border-purple-200 rounded-lg p-4 mb-4 {{ old('has_certificate', $event->has_certificate) ? '' : 'hidden' }}">
                ...
                <div class="mb-3">
                    <p class="text-sm text-gray-600 mb-2">Template saat ini:</p>
                    <img src="{{ asset('storage/' . $event->certificate_template) }}" alt="Certificate Template" class="max-w-full rounded shadow border">
                </div>
                @endif
                
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Upload Template Sertifikat Baru</label>
                    ...
                    <div id="certificateTemplatePreview" class="mt-3 hidden">
                        <p class="text-sm text-gray-600 mb-2">Preview template baru:</p>
                        <img id="templatePreview" src="" alt="Preview Template" class="max-w-full rounded shadow border">
                    </div>
                </div>
            </div>
            
            <div id="registrationDetails" class="{{ old('has_registration', $event->has_registration) ? '' : 'hidden' }}">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Kuota Peserta</label>
                        <input type="number" name="participant_quota" value="{{ old('participant_quota', $event->participant_quota) }}" min="1"
                ...
                <div>
                    ...
                    <textarea name="registration_requirements" rows="3"
                              class="w-full px-4 py-2 border rounded focus:ring-2 focus:ring-[#00629B]"
                              placeholder="Contoh: KTP, CV, Surat Rekomendasi">{{ old('registration_requirements', $event->registration_requirements) }}</textarea>
                </div>
            </div>
        </div>
