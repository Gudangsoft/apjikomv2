...
<div class="bg-white rounded-lg shadow p-6">
    <h3 class="text-2xl font-bold text-gray-900 mb-6">Edit Media Galeri</h3>
    ...
    <form action="{{ route('admin.galleries.update', $gallery) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-6">
            ...
            <textarea id="description" name="description" rows="3"
                      class="w-full px-4 py-2 border rounded focus:ring-2 focus:ring-purple-500 @error('description') border-red-500 @enderror">{{ old('description', $gallery->description) }}</textarea>
            @error('description')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div id="imageField" class="mb-6" style="display: {{ old('type', $gallery->type) == 'image' ? 'block' : 'none' }}">
            <label for="image" class="block text-sm font-medium text-gray-700 mb-2">Gambar</label>
            @if($gallery->type == 'image' && $gallery->image && !str_starts_with($gallery->image, 'http'))
                <div class="mb-3">
                    <img src="{{ asset('storage/' . $gallery->image) }}" alt="Current Image" class="rounded-lg border" style="max-width: 300px; max-height: 200px;">
            ...
            @error('image')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
            <p class="text-xs text-gray-500 mt-1">Format: JPG, JPEG, PNG. Maksimal 5MB. Kosongkan jika tidak ingin mengubah.</p>
        </div>

        <!-- YouTube URL (shown when type=video) -->
        <div id="videoField" class="mb-6" style="display: {{ old('type', $gallery->type) == 'video' ? 'block' : 'none' }}">
            ...
            @if($gallery->type == 'video' && $gallery->youtube_id)
                <div class="mb-3">
                    <img src="{{ $gallery->youtube_thumbnail }}" alt="YouTube Thumbnail" class="rounded-lg border" style="max-width: 300px; max-height: 200px;">
                    <p class="text-xs text-gray-500 mt-1">Thumbnail video saat ini</p>
                </div>
            @endif
            <input type="url" id="youtube_url" name="youtube_url" value="{{ old('youtube_url', $gallery->youtube_url) }}" placeholder="https://www.youtube.com/watch?v=..."
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div>
                ...
                <select id="category" name="category" required
                    ...
                    <option value="activity" {{ old('category', $gallery->category) == 'activity' ? 'selected' : '' }}>Kegiatan</option>
                    <option value="member" {{ old('category', $gallery->category) == 'member' ? 'selected' : '' }}>Member</option>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div class="flex items-center">
                <label class="flex items-center space-x-2">
                    <input type="checkbox" id="is_featured" name="is_featured" value="1" {{ old('is_featured', $gallery->is_featured) ? 'checked' : '' }}
...
function toggleMediaType() {
    ...
    if (imageRadio.checked) {
        ...
        videoInput.required = false; // Not required for edit if not changed
    }
}
