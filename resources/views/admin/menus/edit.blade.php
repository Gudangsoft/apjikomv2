...
@section('content')
<div class="mb-6">
    <h3 class="text-2xl font-bold text-gray-900">Edit Menu</h3>
</div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <select name="type" id="type" 
                    ...
                    required>
                    <option value="link" {{ old('type', $menu->type) == 'link' ? 'selected' : '' }}>Link (URL Custom)</option>
                    <option value="page" {{ old('type', $menu->type) == 'page' ? 'selected' : '' }}>Halaman (Link ke halaman)</option>
                    <option value="dropdown" {{ old('type', $menu->type) == 'dropdown' ? 'selected' : '' }}>Dropdown (Menu dengan sub-menu)</option>
                </select>
                @error('type')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
                ...
                <select name="parent_id" id="parent_id" 
                    ...
                    @foreach($parentMenus as $parentMenu)
                        ...
                        @foreach($parentMenu->children as $childMenu)
                            <option value="{{ $childMenu->id }}" {{ old('parent_id', $menu->parent_id) == $childMenu->id ? 'selected' : '' }}>
