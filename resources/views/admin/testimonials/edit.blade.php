...
<div class="bg-white rounded-lg shadow p-6">
    <h3 class="text-2xl font-bold text-gray-900 mb-6">Edit Testimoni</h3>
    ...
    <form action="{{ route('admin.testimonials.update', $testimonial) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-6">
            ...
            <select id="member_id" name="member_id" required
                ...
                @foreach($members as $member)
                    <option value="{{ $member->id }}" {{ old('member_id', $testimonial->member_id) == $member->id ? 'selected' : '' }}>
            ...
            @enderror
            <p class="text-xs text-gray-500 mt-1">Pilih member yang memberikan testimoni</p>
        </div>
        ...
        <div class="mb-6">
            ...
            <textarea id="content" name="content" rows="5" required
                      class="w-full px-4 py-2 border rounded focus:ring-2 focus:ring-purple-500 @error('content') border-red-500 @enderror">{{ old('content', $testimonial->content) }}</textarea>
            @error('content')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div>
                ...
                @if($testimonial->photo)
                    <div class="mb-3">
                        <img src="{{ asset('storage/' . $testimonial->photo) }}" alt="Current Photo" class="rounded-lg border" style="max-width: 150px; max-height: 150px;">
                        <p class="text-xs text-gray-500 mt-1">Foto saat ini</p>
                    </div>
                @elseif($testimonial->member && $testimonial->member->photo)
                    <div class="mb-3">
                        <img src="{{ asset('storage/' . $testimonial->member->photo) }}" alt="Member Photo" class="rounded-lg border" style="max-width: 150px; max-height: 150px;">
                        <p class="text-xs text-gray-500 mt-1">Foto dari profil member</p>
                    </div>
                @endif
