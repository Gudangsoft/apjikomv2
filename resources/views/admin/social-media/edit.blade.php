...
@section('content')
@php
$knownPlatforms = ['facebook','instagram','twitter','youtube','linkedin','tiktok','whatsapp','telegram'];
$currentName = strtolower($socialMedium->name);
$isKnown = false;
foreach ($knownPlatforms as $kp) {
    if (str_contains($currentName, $kp) || str_contains($currentName, str_replace(' / ', '', $kp))) {
        $isKnown = true; break;
    }
}
@endphp
...
<div class="max-w-3xl">
    ...
    <form action="{{ route('admin.social-media.update', $socialMedium) }}" method="POST" enctype="multipart/form-data" id="smForm">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-xl shadow-sm">
            ...
            <div class="p-5 border-b flex items-center gap-3">
                <div id="headerIcon" class="w-10 h-10 rounded-xl flex items-center justify-center shadow-sm flex-shrink-0">
                    @php
                    $p = strtolower($socialMedium->name);
                    $colors = ['facebook'=>'#1877F2','instagram'=>'#E1306C','twitter'=>'#000000','youtube'=>'#FF0000','linkedin'=>'#0A66C2','tiktok'=>'#010101','whatsapp'=>'#25D366','telegram'=>'#26A5E4'];
                    $bgColor = '#6366f1';
                    foreach ($colors as $k => $v) { if (str_contains($p, $k)) { $bgColor = $v; break; } }
                    @endphp
                    @if($socialMedium->icon)
                        <img src="{{ asset('storage/' . $socialMedium->icon) }}" class="w-8 h-8 object-contain rounded" alt="{{ $socialMedium->name }}">
                    @else
                        <svg class="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 24 24">
                            @if(str_contains($p, 'facebook'))
                                <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                            @elseif(str_contains($p, 'instagram'))
                ...
                <div>
                    <h1 class="text-lg font-bold text-gray-900">{{ $socialMedium->name }}</h1>
                    <p class="text-sm text-gray-500">Edit informasi media sosial</p>
                </div>
            </div>
            ...
            <div class="p-6 space-y-6">
                ...
                {{-- Platform Picker --}}
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-3">Ganti Platform</label>
                    <div class="grid grid-cols-3 sm:grid-cols-5 gap-3" id="platformGrid">
                        ...
                        $platforms = [
                            ...
                            ['name' => 'Instagram', 'color' => '#E1306C', 'svg' => '<path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/>'],
                        ...
                        @php $isActive = str_contains(strtolower($socialMedium->name), strtolower(explode(' ', $plt['name'])[0])); @endphp
                        <button type="button"
                            onclick="selectPlatform('{{ $plt['name'] }}', '{{ $plt['color'] }}')"
                            data-name="{{ $plt['name'] }}"
                            class="platform-tile group flex flex-col items-center gap-2 p-3 rounded-xl border-2 transition-all cursor-pointer
                                {{ $isActive ? 'border-purple-500 bg-purple-50 ring-2 ring-purple-300' : 'border-gray-200 hover:border-purple-400 hover:bg-purple-50' }}">
                            <div class="w-12 h-12 rounded-xl flex items-center justify-center shadow-sm" style="background-color: {{ $plt['color'] }}">
                                <svg class="w-6 h-6 text-white" fill="currentColor" viewBox="0 0 24 24">{!! $plt['svg'] !!}</svg>
                            </div>
                            <span class="text-xs font-medium text-center leading-tight {{ $isActive ? 'text-purple-600' : 'text-gray-600 group-hover:text-purple-600' }}">{{ $plt['name'] }}</span>
                        </button>
                        @endforeach
                        ...
                        <button type="button"
                            ...
                            data-name="custom"
                            class="platform-tile group flex flex-col items-center gap-2 p-3 rounded-xl border-2 transition-all cursor-pointer
                                {{ !$isKnown ? 'border-purple-500 bg-purple-50 ring-2 ring-purple-300' : 'border-gray-200 hover:border-purple-400 hover:bg-purple-50' }}">
                            <div class="w-12 h-12 rounded-xl {{ !$isKnown ? 'bg-gray-400' : 'bg-gray-200' }} flex items-center justify-center shadow-sm">
                                <svg class="w-6 h-6 {{ !$isKnown ? 'text-white' : 'text-gray-500 group-hover:text-gray-700' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>
                                </svg>
                            </div>
                            <span class="text-xs font-medium text-center leading-tight {{ !$isKnown ? 'text-purple-600' : 'text-gray-600 group-hover:text-purple-600' }}">Custom</span>
                        </button>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $socialMedium->name) }}" required
                ...
                <div>
                    ...
                    <input type="url" name="url" value="{{ old('url', $socialMedium->url) }}" required
                ...
                {{-- Custom Icon --}}
                <div id="customIconSection" class="{{ $isKnown ? 'hidden' : '' }}">
                    ...
                    <div class="flex items-center gap-3 mb-3 p-3 bg-gray-50 rounded-lg">
                        <img src="{{ asset('storage/' . $socialMedium->icon) }}" class="w-10 h-10 object-contain rounded border" alt="Icon saat ini">
                        <div>
                            <p class="text-xs font-medium text-gray-700">Icon saat ini</p>
                    ...
                    <div class="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center hover:border-purple-400 transition-colors">
                        ...
                        <div id="iconUploadPlaceholder" onclick="document.getElementById('iconUpload').click()" class="cursor-pointer">
                            ...
                            <p class="text-sm text-gray-600 font-medium">{{ $socialMedium->icon ? 'Upload icon baru' : 'Klik untuk upload icon' }}</p>
                        ...
                        <div id="iconPreviewBox" class="hidden">
                            ...
                            <button type="button" onclick="clearIcon()" class="mt-2 text-xs text-red-500 hover:text-red-700">Hapus pilihan</button>
                        </div>
                    </div>
                    ...
                    <div class="mt-3">
                        ...
                        <div class="flex items-center gap-3">
                            <input type="color" name="icon_class" id="iconColor"
                                value="{{ old('icon_class', $socialMedium->icon_class ?: '#6366f1') }}"
                ...
                <div>
                    ...
                    <input type="text" name="note" value="{{ old('note', $socialMedium->note) }}"
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="order" value="{{ old('order', $socialMedium->order) }}" min="0"
