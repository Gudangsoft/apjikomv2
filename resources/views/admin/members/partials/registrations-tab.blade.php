...
<div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
    ...
    <div class="mb-4 grid grid-cols-1 md:grid-cols-3 gap-4">
        <form method="GET" action="{{ route('admin.members.index') }}" class="md:col-span-2">
        ...
        <form method="GET" action="{{ route('admin.members.index') }}" class="flex gap-2">
            ...
            <select name="reg_status" onchange="this.form.submit()" class="flex-1 px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500">
                ...
                <option value="pending" {{ request('reg_status') == 'pending' ? 'selected' : '' }}>Pending</option>
                <option value="approved" {{ request('reg_status') == 'approved' ? 'selected' : '' }}>Approved</option>
                <option value="rejected" {{ request('reg_status') == 'rejected' ? 'selected' : '' }}>Rejected</option>
            </select>
        </form>
    </div>
    ...
    <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50 transition" data-registration-id="{{ $registration->id }}" data-status="{{ $registration->status }}">
                    <td class="px-4 py-4 text-center">
                        @if($registration->status == 'pending')
                            <input type="checkbox" class="registration-checkbox w-4 h-4 text-purple-600 border-gray-300 rounded focus:ring-purple-500" value="{{ $registration->id }}" onchange="updateBulkActionsBar()">
                        @else
                    ...
                    <td class="px-4 py-4">
                        <div class="flex items-center">
                            @if($registration->photo)
                                <img src="{{ asset('storage/' . $registration->photo) }}" class="w-10 h-10 rounded-full object-cover mr-3">
                            @else
                                <div class="w-10 h-10 rounded-full bg-purple-100 flex items-center justify-center mr-3">
                                    <span class="text-purple-600 font-bold">{{ substr($registration->full_name, 0, 1) }}</span>
                                </div>
                            @endif
                            <div>
                                <div class="text-sm font-semibold text-gray-900">{{ $registration->full_name }}</div>
                                <div class="text-xs text-gray-500">{{ $registration->institution ?? 'N/A' }}</div>
                            </div>
                        </div>
                    </td>
                    <td class="px-4 py-4">
                    ...
                    <td class="px-4 py-4 text-right">
                        <div class="flex items-center justify-end gap-2">
                            ...
                            @if($registration->status == 'pending')
                            <!-- Approve -->
                            <form action="{{ route('admin.registrations.update-status', $registration) }}" method="POST" class="inline">
                                @csrf
                                @method('PUT')
    ...
    <div class="mt-6 flex items-center justify-between border-t border-gray-200 pt-4">
        <div class="text-sm text-gray-700">
            Menampilkan <span class="font-semibold">{{ $registrations->firstItem() }}</span>
            sampai <span class="font-semibold">{{ $registrations->lastItem() }}</span>
            dari <span class="font-semibold">{{ $registrations->total() }}</span> pendaftaran
        </div>
        <div>
            {{ $registrations->appends(['tab' => 'registrations'])->links() }}
        </div>
    </div>
    @endif
</div>
...
function bulkApprove() {
    ...
    form.method = 'POST';
    form.action = '{{ route('admin.registrations.bulk-action') }}';
