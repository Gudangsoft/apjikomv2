...
<div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
    ...
    <div class="mb-4 flex justify-between items-center">
        <div class="text-sm text-gray-600">
            Total: <span class="font-semibold">{{ $members->total() ?? 0 }}</span> member
        </div>
        <div class="flex gap-2">
            <a href="{{ route('admin.members.export', array_merge(request()->all(), ['format' => 'csv', 'tab' => 'members'])) }}" 
            ...
            <a href="{{ route('admin.members.export', array_merge(request()->all(), ['format' => 'excel', 'tab' => 'members'])) }}"
            ...
            <a href="{{ route('admin.members.export', array_merge(request()->all(), ['format' => 'pdf', 'tab' => 'members'])) }}"
                ...
                <tr class="hover:bg-gray-50 transition">
                    <td class="px-4 py-4">
                        <div class="flex items-center">
                            @if($member->photo)
                                <img src="{{ asset('storage/' . $member->photo) }}" class="w-10 h-10 rounded-full object-cover mr-3">
                            @else
                                <div class="w-10 h-10 rounded-full bg-purple-100 flex items-center justify-center mr-3">
                                    <span class="text-purple-600 font-bold">{{ substr($member->user->name, 0, 1) }}</span>
                                </div>
                            @endif
                            <div>
                                <div class="text-sm font-semibold text-gray-900">{{ $member->user->name }}</div>
                                <div class="text-xs text-gray-500">{{ $member->institution_name ?? 'N/A' }}</div>
                                @if($member->member_number)
                                    <div class="text-xs text-purple-600 font-mono">{{ $member->member_number }}</div>
                                @endif
                            </div>
                        </div>
                    </td>
                    ...
                    <td class="px-4 py-4">
                        <div class="text-sm text-gray-900">{{ $member->city ?? '-' }}</div>
                        <div class="text-xs text-gray-500">{{ $member->province ?? '' }}</div>
                    </td>
                    <td class="px-4 py-4 text-center">
                        ...
                        @else
                            <span class="px-3 py-1 text-xs font-semibold rounded-full bg-red-100 text-red-700">{{ ucfirst($member->status) }}</span>
                        @endif
                    </td>
                    <td class="px-4 py-4 text-center">
                    ...
                    <td class="px-4 py-4 text-right">
                        <div class="flex items-center justify-end gap-2">
                            ...
                            <a href="{{ route('admin.members.card', $member) }}" target="_blank"
                               class="p-2 text-purple-600 hover:bg-purple-50 rounded-lg transition"
                               title="Kartu / Download">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="mt-6 flex items-center justify-between border-t border-gray-200 pt-4">
        <div class="text-sm text-gray-700">
            Menampilkan <span class="font-semibold">{{ $members->firstItem() }}</span>
            sampai <span class="font-semibold">{{ $members->lastItem() }}</span>
            dari <span class="font-semibold">{{ $members->total() }}</span> member
        </div>
        <div>
