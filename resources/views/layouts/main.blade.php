...
<body class="antialiased bg-gray-50 dark:bg-dark-900 text-gray-900 dark:text-gray-100">
        ...
        <div class="container mx-auto px-4">
            <div class="flex justify-between items-center py-3">
                ...
                <div class="md:hidden flex items-center gap-2">
                    ...
                    <button class="text-gray-700 dark:text-gray-300 p-2" id="mobile-menu-button">
        ...
        <div class="container mx-auto px-4 py-12">
            <div class="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-8">
                <div class="col-span-2 md:col-span-1">
                    ...
                    @if($globalSocialMedia->isNotEmpty())
                    <div class="flex space-x-3">
                        @foreach($globalSocialMedia as $sm)
                        @php $p = strtolower($sm->name); @endphp
                        <a href="{{ $sm->url }}" target="_blank" title="{{ $sm->name }}"
                            class="w-8 h-8 rounded-full flex items-center justify-center hover:opacity-80 transition"
                            style="background-color: {{ ($sm->icon_class && str_starts_with($sm->icon_class, '#')) ? $sm->icon_class.'33' : 'rgba(255,255,255,0.1)' }}">
                            @if($sm->icon)
                                <img src="{{ asset('storage/' . $sm->icon) }}" class="w-4 h-4 object-contain rounded" alt="{{ $sm->name }}">
                            @elseif(str_contains($p, 'facebook'))
    ...
    <script>
        ...
        // Toggle dark mode (desktop + mobile buttons)
        [darkModeToggle, document.getElementById('darkModeToggleMobile')].forEach(btn => {
            if (btn) {
